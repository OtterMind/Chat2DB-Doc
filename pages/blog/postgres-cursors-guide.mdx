...
For stateless APIs, use [keyset pagination](/blog/keyset-pagination-postgres-guide). Cursors win inside *one job*: ETL, exports, report generation — anywhere a single session drains a huge, consistent result.
...
- Long-lived cursors show up as long-running transactions in monitoring: check `pg_stat_activity.backend_xid`/`xmin` age, and see our [kill/cancel guide](/blog/postgres-kill-query-cancel-terminate) when one must die.
