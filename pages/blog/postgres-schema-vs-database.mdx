...
1. **Shared schema, tenant_id column + Row-Level Security.** One set of tables; RLS policies enforce isolation. Cheapest to operate, migrations run once, and it scales to huge tenant counts. See our [row-level security guide](https://chat2db.ai/blog/postgres-row-level-security-guide).
