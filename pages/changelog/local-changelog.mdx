...
## 5.3.7-beta.4
`2026-09-21`
- 🔥【New Feature】Running `EXPLAIN PLAN` in the Oracle SQL console now returns the execution plan instead of reporting success with nothing to show, and a missing `PLAN_TABLE` reports an actionable message.
- 🐞【Fixed】Fixed in-app updates that stopped after the application closed on some macOS machines: the update helper now runs independently of the application, the application waits until the helper has accepted the plan before closing, and the update restarts into the new version.
- 🐞【Fixed】Fixed desktop installs that replace the whole package (macOS, Linux AppImage) never returning to a usable version when an update fails: the previous version is now restored and reopened, and switching is refused with a failure reason while another instance is running.
...
- 🐞【Fixed】Fixed the local data source list applying search and type filters before paging, which made searches ineffective or produced empty pages.
- 🐞【Fixed】Fixed local data sources, dashboards, and other local settings updating the UI before they were persisted, which could lose or duplicate them after a restart.
- 🐞【Fixed】Fixed ER diagram positions, namespaces, and group positions becoming mixed up or reverting when a save failed or operations ran concurrently.
