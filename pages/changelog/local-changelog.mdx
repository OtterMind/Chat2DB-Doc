...
- 🔥【New Feature】SQL execution now supports progressive result return, so long-running SQL and continuous execution can show progress and results sooner.
...
- 🔥【New Feature】Added the Chat2DB CLI runtime and CLI Skill. The CLI can locate Local runtime status, ports, logs, and tokens, and can query data sources, read metadata, test connections, and execute SQL.
...
- 🔥【New Feature】Added network proxy settings, making database connections easier in complex network environments.
...
- ⭐【Optimization】MCP no longer needs to be bound to a single data source configuration. It can directly view and operate all local data sources, then choose the database and schema for the current task.
